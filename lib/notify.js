...
async function writeState(file, data) {
  ...
  // unique tmp per write: concurrent writers (broadcast pruning while a
  // subscribe POST lands) must not interleave one writer with another's rename
...
export class PushStore {
  ...
  async broadcast(payload, opts = {}) {
    ...
    await Promise.all([...this.subs.values()].map(async (sub) => {
      ...
      let host;
      try {
        // inside the try: one malformed stored endpoint must not reject the
        // whole broadcast (it used to 500 /api/push/test for everyone)
        host = `${new URL(sub.endpoint).host}…${sub.endpoint.slice(-6)}`;
        ...
        // host undefined ⇒ the endpoint itself was unparseable — prune it,
        // it can never deliver
        if (host === undefined) dead.push(sub.endpoint);
        results.push({ host: host ?? String(sub.endpoint).slice(0, 40), status: 0, error: String(e.message ?? e) });
...
export class Coordinator {
  ...
  async enrich(msg, paneId) {
    ...
    if (ctx.kind === 'ask' && ctx.questions?.length) {
      ...
      // no answer buttons: the plan prompt's feedback row swallows digit
      // keystrokes, so answering safely needs the app's cursor navigation
      msg.body = ctx.question || '📋 plan ready — approve?';

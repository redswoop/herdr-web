...
// hints: { pid?, sessionId? }. Correlation strength, strongest first:
...
//   3. cwd + live pid + newest opened_at — the old heuristic, still needed
//      when /proc isn't available to resolve the pane's pid.
...
function grokTranslateUpdate(obj) {
  ...
  switch (u.sessionUpdate) {
    ...
    case 'tool_call_update': {
      ...
      return [{ kind: 'tool_result', text: grokContentText(u.content), ts, id: u.toolCallId }];
    }
    case 'turn_completed': {
      const us = u.usage;
